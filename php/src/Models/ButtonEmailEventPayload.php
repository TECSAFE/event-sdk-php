...
final class ButtonEmailEventPayload implements \Tecsafe\OFCP\Events\OfcpEvent
{
     ...
     * Payload for sending emails with one to two buttons
     * 
     * @param Body $body Email body attributs text: Email text that will be added in the template {{ text }} without html tags Links are parsed to anchor tags title: Email title that will be added in the template {{ title }} not the subject data: holds the data for the buttons
     ...
     * @return self
     */
    public function __construct(private Body $body, private Header $header)
    {
    }
    /**
    ...
    public static function from_json(string|array $json): self
    {
        ...
        return new self(isset($data['body']) ? Body::from_json($data['body']) : null, isset($data['header']) ? Header::from_json($data['header']) : null);
    }
